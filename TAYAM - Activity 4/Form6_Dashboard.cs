...
namespace TAYAM___Activity_4
{
    public partial class Form6 : Form
    {
        ...
        {
            int totalEmployeesExceptID1 = GetTotalEmployeesExceptID1();
            label6.Text = $"{totalEmployeesExceptID1}";

            int totalPresentEmployees = GetTotalPresentEmployees();
            label9.Text = $"{totalPresentEmployees}";

            //int totalAbsentEmployees = GetTotalAbsentEmployees();
            //label13.Text = $"{totalAbsentEmployees}";

            int leaveCount = GetLeaveCount();
            label11.Text = $"{leaveCount}";
        }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            // Update the label with the current date in the desired format
            label17.Text = DateTime.Now.ToString("dddd, MMMM d, yyyy");
        }
        ...
        private void monthCalendar1_DateChanged(object sender, DateRangeEventArgs e)
        {
            ...
            monthCalendar1.Text = $"Selected Date: {e.Start.ToShortDateString()}";
        ...
        private int GetTotalEmployeesExceptID1()
        {
            int totalEmployees = 0;
            string myConnectionString = "server=localhost;uid=root;pwd=;database=edpempattendance";
            string query = "SELECT COUNT(*) FROM employees WHERE ID != 1";
        ...
        private int GetTotalPresentEmployees()
        {
            int totalPresentEmployees = 0;

            DateTime currentDate = DateTime.Now.Date;

            string query = $"SELECT COUNT(*) FROM attendance_entries WHERE DATE(entry_date) = DATE('{currentDate.ToString("yyyy-MM-dd")}')";
            ...
            using (MySqlConnection connection = new MySqlConnection(myConnectionString))
            {
                ...
                {
                    connection.Open();
                    totalPresentEmployees = Convert.ToInt32(command.ExecuteScalar());
                }
            }

            return totalPresentEmployees;
        }
        ...
        private int GetTotalAbsentEmployees()
        {
            ...
            string myConnectionString = "server=localhost;uid=root;pwd=;database=edpempattendance";
            string query = $"SELECT COUNT(*) FROM employees WHERE TotalAbsent > 0 AND ID NOT IN (SELECT employee_id FROM attendance_entries WHERE DATE(entry_date) = DATE('{currentDate.ToString("yyyy-MM-dd")}'))";
            ...
            using (MySqlConnection connection = new MySqlConnection(myConnectionString))
            {
                ...
                {
                    connection.Open();
                    totalAbsentEmployees = Convert.ToInt32(command.ExecuteScalar());
                }
            }

            return totalAbsentEmployees;
        }
        ...
        private int GetLeaveCount()
        {
            ...
            string myConnectionString = "server=localhost;uid=root;pwd=;database=edpempattendance";
            string query = "SELECT COUNT(*) FROM leave_forms WHERE status = 'Approved'";
            ...
            using (MySqlConnection connection = new MySqlConnection(myConnectionString))
            {
                ...
                {
                    connection.Open();
                    leaveCount = Convert.ToInt32(command.ExecuteScalar());
                }
            }
